...
(deftest char->key-code-test
  (testing "letters map to uppercase AWT key codes"
    (is (= KeyEvent/VK_A (robot/char->key-code \a)))
    (is (= KeyEvent/VK_Z (robot/char->key-code \z)))
    (is (= KeyEvent/VK_M (robot/char->key-code \m))))
  (testing "space maps to VK_SPACE"
    (is (= KeyEvent/VK_SPACE (robot/char->key-code \space))))
  (testing "slash maps to VK_SLASH"
    (is (= KeyEvent/VK_SLASH (robot/char->key-code \/))))
  (testing "case fallthrough for non-special chars"
    (is (= (Character/toUpperCase (int \1)) (robot/char->key-code \1)))
    (is (= (Character/toUpperCase (int \.)) (robot/char->key-code \.)))))
...
(deftest send-message-test
  (testing "sends \"/all nice tutorial\" with default config"
    (let [calls (atom []) robot (recording-robot calls)]
      (robot/send-message robot default-test-config)
      (is (= [[:delay 200]
  ...
  (testing "sends \"/team nice tutorial\" with team chat config"
    (let [calls (atom []) robot (recording-robot calls)]
      (robot/send-message robot (assoc default-test-config :chat-mode :team))
  ...
  (testing "sends custom message"
    (let [calls (atom []) robot (recording-robot calls)]
      (robot/send-message robot (assoc default-test-config :message "gg wp"))
  ...
  (testing "uses create-robot when called without args"
    (let [called (atom false)]
      (with-redefs [robot/create-robot (fn [] (reset! called true) (recording-robot (atom [])))]
        (robot/send-message default-test-config))
      (is called))))
...
(deftest listener-dispatch-test
  (testing "triggers on matching numpad 1 event"
    (let [sent? (atom false)
          listener (listener/create-listener default-test-config)
          event (make-num-event NativeKeyEvent/VC_1 NativeKeyEvent/KEY_LOCATION_NUMPAD)]
      (with-redefs [robot/send-message (fn [_] (reset! sent? true))]
  ...
  (testing "uses configurable key-code and key-location"
    (let [sent? (atom false)
          ...
          listener (listener/create-listener custom-config)
...
(deftest lifecycle-test
  (testing "start! sets running? to true and stores listener"
    (let [orig @sut/app-state]
      ...
      (with-redefs [listener/add-listener! (fn [_])]
        (sut/start!)
  ...
  (testing "stop! clears running? and listener"
    (let [mock-listener (Object.)
      ...
      (with-redefs [listener/remove-listener! (fn [_])]
        (sut/stop!)
